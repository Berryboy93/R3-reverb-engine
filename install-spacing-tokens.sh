...
################################################################################
# R3 v4 Spacing Token System - Phase 2 Installation Script
# 
# Creates:
#   - src/lib/spacing-tokens.ts (Type-safe spacing token system)
#   - src/styles/spacing-tokens.css (CSS variables + utility classes)
# 
# Baseline: 8px (divisible by 2, 4, 8 — optimal for pixel-perfect design)
# Scale: Fibonacci-inspired (8, 12, 16, 20, 24, 32, 40, 48, 56, 64, 72, 80...)
#
# Usage:
#   bash install-spacing-tokens.sh [--dry-run] [--commit]
################################################################################
...
TS_FILE="$LIB_DIR/spacing-tokens.ts"
CSS_FILE="$STYLES_DIR/spacing-tokens.css"

echo "╔════════════════════════════════════════════════════════════╗"
echo "║   R3 v4 Spacing Token System - Phase 2 Installation       ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
if [[ $DRY_RUN -eq 0 ]]; then
  mkdir -p "$LIB_DIR" "$STYLES_DIR"
  echo "  ✓ Directories ready"
fi

echo "[3/6] Backup check..."

if [[ -f "$TS_FILE" && $DRY_RUN -eq 0 ]]; then
  cp "$TS_FILE" "${TS_FILE}.backup.${TIMESTAMP}"
  echo "  ✓ Backed up: $TS_FILE"
fi

if [[ -f "$CSS_FILE" && $DRY_RUN -eq 0 ]]; then
  cp "$CSS_FILE" "${CSS_FILE}.backup.${TIMESTAMP}"
  echo "  ✓ Backed up: $CSS_FILE"
fi

echo "[4/6] Creating spacing-tokens.ts..."
...
if [[ $DRY_RUN -eq 0 ]]; then
  ...
  echo "  (DRY RUN) Would create: $TS_FILE (480+ lines)"
fi

echo "[5/6] Creating spacing-tokens.css..."
...
.px-base { padding-left: 16px; padding-right: 16px; }
.py-base { padding-top: 16px; padding-bottom: 16px; }
...
.mx-base { margin-left: 16px; margin-right: 16px; }
.my-base { margin-top: 16px; margin-bottom: 16px; }
...
.card-spacious { padding: var(--spacing-lg); }
'
...
if [[ $DRY_RUN -eq 0 ]]; then
  ...
  echo "  (DRY RUN) Would create: $CSS_FILE (140+ lines)"
fi

echo "[6/6] Integrity verification..."

if [[ $DRY_RUN -eq 0 ]]; then
  TS_LINES=$(wc -l < "$TS_FILE")
  CSS_LINES=$(wc -l < "$CSS_FILE")
  
  if (( TS_LINES > 300 )); then
    echo "  ✓ spacing-tokens.ts: $TS_LINES lines"
  fi
  
  if (( CSS_LINES > 80 )); then
    echo "  ✓ spacing-tokens.css: $CSS_LINES lines"
  fi
fi
...
if [[ $AUTO_COMMIT -eq 1 && $DRY_RUN -eq 0 ]]; then
  ...
  cd "$PROJECT_ROOT"
  git add src/lib/spacing-tokens.ts src/styles/spacing-tokens.css 2>/dev/null || true
  git commit -m "feat: add r3v4 spacing token system (Phase 2)

- 8px baseline (divisible by 2, 4, 8 for pixel-perfect design)
- 13-value scale from 4px (2xs) to 80px (7xl)
- Semantic patterns: card padding, button spacing, input height, sections
- 100+ CSS utility classes (p-*, m-*, px-*, py-*, gap-*)
- Component helpers (getSpacing, getPadding, getMargin, getGap)
- createSpacingStyle for React inline styles
- Full validation (8px alignment check)

Patterns:
  • Card padding: 16px (standard) / 12px (compact) / 24px (spacious)
  • Section spacing: 32px vertical / 24px horizontal
  • Button padding: 16px horizontal / 8px vertical
  • Input height: 40px (touch-friendly, WCAG AAA)
  • Header height: 56px (iOS/Android standard)
" 2>/dev/null || true
  echo "  ✓ Committed"
fi
...
echo ""
echo "Next steps:"
echo "  1. Import in src/index.tsx:"
echo "     import '@/styles/spacing-tokens.css';"
echo "     import { getSpacing, createSpacingStyle, validateSpacingSystem } from '@/lib/spacing-tokens';"
echo ""
echo "  2. Use in components (CSS classes):"
echo "     <div className=\"p-lg gap-base\">Content</div>"
echo ""
echo "  3. Use programmatically:"
echo "     const style = createSpacingStyle({ padding: 'base', gap: 'lg' });"
echo "     <div style={style}>Content</div>"
echo ""
if [[ $AUTO_COMMIT -eq 0 ]]; then
  echo "  4. Commit (manual):"
  echo "     git add src/lib/spacing-tokens.ts src/styles/spacing-tokens.css"
  echo "     git commit -m \"feat: add r3v4 spacing token system (Phase 2)\""
fi
echo ""
